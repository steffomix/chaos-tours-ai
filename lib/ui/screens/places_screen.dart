...
class _PlacesScreenState extends State<PlacesScreen> {
  ...
  Future<void> _loadPlaces() async {
    ...
    for (final p in places) {
      if (p.id != null) {
        counts[p.id!] =
            await DatabaseService.instance.visitCountForPlace(p.id!);
        last[p.id!] =
            await DatabaseService.instance.lastVisitedAtForPlace(p.id!);
      }
    }
    if (mounted) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _loading
          ...
          : _places.isEmpty
              ? const Center(child: Text('Keine Orte gespeichert.\nOrte auf der Karte per Langer Druck hinzufügen.', textAlign: TextAlign.center))
              : RefreshIndicator(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (ctx, i) {
                      ...
                      final last =
                          place.id != null ? _lastVisited[place.id] : null;
                      return ListTile(
                        leading: CircleAvatar(
                          backgroundColor: place.placeType.dotColor
