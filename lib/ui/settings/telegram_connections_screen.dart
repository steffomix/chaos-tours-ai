...
import 'telegram_connection_detail_screen.dart';
...
class _TelegramConnectionsScreenState extends State<TelegramConnectionsScreen> {
  ...
  Future<void> _add() async {
    final saved = await Navigator.push<bool>(
      ...
      MaterialPageRoute(
        ...
        builder: (_) => const TelegramConnectionDetailScreen(existing: null),
      ),
    );
  ...
  Future<void> _edit(TelegramConnection conn) async {
    final saved = await Navigator.push<bool>(
      context,
      MaterialPageRoute(
        builder: (_) => TelegramConnectionDetailScreen(existing: conn),
      ),
    );
    if (saved == true) await _load();
  }
